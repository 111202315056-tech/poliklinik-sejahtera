...
<p style="font-size:18px;font-weight:700;color:#1e293b;">Rp {{ number_format($d->periksa->biaya_periksa,0,',','.') }}</p>
</div>
</div>
@if($d->periksa->detailPeriksa->count() > 0)
...
<span style="background:white;border:1px solid #fde68a;color:#92400e;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:500;">{{ $dp->obat->nama_obat ?? '-' }} ({{ $dp->obat->kemasan ?? '' }})</span>
@endforeach
</div>
</div>
@else
<div style="background:#f8fafc;border-radius:8px;padding:12px;font-size:13px;color:#94a3b8;"><i class="fa fa-circle-info" style="margin-right:6px;"></i>Tidak ada obat yang diberikan</div>
@endif
<p style="font-size:11px;color:#94a3b8;margin-top:10px;">Diperiksa pada: {{ $d->periksa->tgl_periksa }}</p>
</div>
@else
...
@empty
<div style="background:white;border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,0.08);padding:60px;text-align:center;">
<i class="fa fa-file-medical" style="font-size:48px;color:#cbd5e1;display:block;margin-bottom:12px;"></i>
<p style="color:#94a3b8;font-size:14px;">Belum ada riwayat pemeriksaan</p>
